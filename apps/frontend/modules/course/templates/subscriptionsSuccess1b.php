...
<style>
.dataTables_wrapper .dataTables_filter { float: left; text-align: right; display: block;  margin-top: 38px;  left: 0px;  margin-bottom: 30px; position: relative; width: 100%; }
	...
	foreach($products_details as $products_detail_key=>$products_detail_val)
	{
		...
		?>
			<div class="col-md-4">
				...
				<u><?php echo "$ " . $products_detail_val["cost"] . " per month"; ?></u>&nbsp; <strong>&nbsp;-&nbsp;<?php echo isset($lib_ctlg_crse_count[$products_detail_val["short_name"]]) ? $lib_ctlg_crse_count[$products_detail_val["short_name"]] : 0; ?> courses</strong>
			</div>
		<?php
		if($loop_i%3 == 0) {
		...
		<div class="panel panel-info">
			<div class="panel-heading">
				<div class="row">
					...
					if(isset($reg_paid_status[$products_detail_key])) {
						if($reg_paid_status[$products_detail_key]["paid_flag"] == 0 && $reg_paid_status[$products_detail_key]["register_flag"] == 0) {
							if($products_detail_val["cost"] > 0) {
								$button_flag = 1;
							} else {
								$button_flag = 2;
							}
						} else if($reg_paid_status[$products_detail_key]["paid_flag"] == 1 && $reg_paid_status[$products_detail_key]["register_flag"] == 0) {
							$button_flag = 2;
						} else if($reg_paid_status[$products_detail_key]["paid_flag"] == 0 && $reg_paid_status[$products_detail_key]["register_flag"] == 1) {
							if($products_detail_val["cost"] > 0) {
								$button_flag = 1;
							}	
						}
					...
					<div class="col-md-3 col-md-offset-3" style="padding-top:20px;">
						<?php if($button_flag == 1) { ?>
							<a class="btn btn-info" href="https://<?php echo $CFG->current_app->getShortName(); ?>.globalclassroom.us/purchase/subscriptionPurchase?institution=<?php echo $products_list_institution[$products_detail_key]; ?>&type=1">
			...
			<div id="collapseOneCS<?php echo $products_detail_val["id"]; ?>" class="panel-collapse collapse" aria-expanded="false" style="height: 0px;">
				<div class="panel-body">
					...
					//}
					?>
					<h4>Courses Catalog: <a data-toggle="collapse" data-parent="#accordion" href="#cert_collapseCourse_<?php echo $ctlg_crse_list_key; ?>"><span style="color:red;"><?php echo $ctlg_crse_list_val; ?> ($)</span>&nbsp;<i class="fa fa-folder-open-o"></i></a></h4>
