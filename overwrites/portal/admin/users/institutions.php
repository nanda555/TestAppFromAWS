...
EOF;
    ...
    //TO:
    $smarty->assign('siteadmin', $CFG->current_app->hasPrivilege('EschoolAdmin'));
    // END OVERWRITE 3
...
// OVERWRITE 8: replacement, changed from:
//if ($institution && $institution != 'mahara') {
// TO:
if ($institution && $institution != 'mahara' && $institution != 'home') {
// END OVERWRITE 8
    $_institution = get_record('institution', 'name', $institution);
    $suspended = $_institution->suspended;
    // OVERWRITE 9: replacement, changed from:
    //if ($USER->get('admin')) {
    // TO:
    global $CFG;
    if (!$CFG->current_app->hasPrivilege('EschoolAdmin')) {
    // END OVERWRITE 9    
        function institution_suspend_submit(Pieform $form, $values) {
            global $SESSION, $USER;
            // OVERWRITE 10: replacement, changed from
            //if (!$USER->get('admin')) {
            // TO:
            if (!$CFG->current_app->hasPrivilege('EschoolAdmin')) {
            // END OVERWRITE 10 
        ...
        function institution_unsuspend_submit(Pieform $form, $values) {
            global $SESSION, $USER;
            // OVERWRITE 11: replacement, changed from:
            //if ($USER->get('admin')) {
            // TO:
            global $CFG;
            if (!$CFG->current_app->hasPrivilege('EschoolAdmin')) {
