...
// TO:
if ($usertype == 'nonmembers' || $usertype == 'lastinstitution' && $CFG->current_app->hasPrivilege('EschoolAdmin')) {
// END OVERWRITE 1
...
function institutionusers_submit(Pieform $form, $values) {
    ...
    //TO:
        ...
        $action = (!empty($values['add']) && $CFG->current_app->hasPrivilege('EschoolAdmin')) ? 'addUserAsMember' : 'inviteUser';
    }
    else if ($values['usertype'] == 'lastinstitution') {
        $action = (!empty($values['add']) && $CFG->current_app->hasPrivilege('EschoolAdmin')) ? 'addUserAsMember' : 'inviteUser';
    }
