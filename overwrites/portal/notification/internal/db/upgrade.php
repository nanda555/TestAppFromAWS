...
function xmldb_notification_internal_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2012021000) {
        ...
        db_create_trigger(
            ...
            'AFTER', 'INSERT', 'notification_internal_activity', '
            IF NEW.read = 0 THEN
                EXECUTE \'UPDATE \' || quote_ident(TG_TABLE_SCHEMA) || \'.\' || \'{usr} SET unread = unread + 1 WHERE id = \' || NEW.usr;
            END IF;'
        );
        db_create_trigger(
            'update_unread_update',
        ...
        db_create_trigger(
            ...
            'AFTER', 'DELETE', 'notification_internal_activity', '
            IF OLD.read = 0 THEN
                EXECUTE \'UPDATE \' || quote_ident(TG_TABLE_SCHEMA) || \'.\' || \'{usr} SET unread = unread - 1 WHERE id = \' || OLD.usr;
            END IF;'
        );
        // END OVERWRITE 1
    }
