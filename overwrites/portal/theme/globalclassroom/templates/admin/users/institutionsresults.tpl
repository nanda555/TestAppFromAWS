        ...
        <tr class="{cycle values='r0,r1'}">
                <td>
                    <a href="{$WWWROOT}institution/index.php?institution={$institution->name}">
                        ...
                        {* OVERWRITE 1: INSERT *}
                        {if $institution->suspended}<span class="suspended"> ({str tag="suspendedinstitution" section=admin}) </span>{/if}
                        {* END OVERWRITE 1 *}
                    </a>
                </td>
                <td class="center">
                ...
                <td class="controls right">
                    {$app = gcr::getApp()}
                    {if $app->getConfigVar('gc_restrict_access_by_institution')}
                    <div class="left">
                        <a href="{$WWWROOT}artefact/eschooladmin/institutioncatalogs.php?institution={$institution->name}">
                            <img title="Edit Catalog Access" src="{theme_url filename="images/btn_access.png"}" />
                        </a>
                    </div>
                    {/if}
                </td>
