<?php
/*
 * Overwrite Update: October 28, 2010 by Ron Stewart
 * Modifications:
 * 1. I changed the syntax of setting the schema so that it uses the built-in find_schema() method
 * to set the $schema variable, aside from cleanliness, this change doesn't affect anything.
 *
 * 2. I discovered that schema support was not being offered for the SQL queries $this->metaKeysSQL and
 * $this->metaDefaultsSQL, resulting in redundant results for each schema in the database. I wrote alternative
 * SQL statements for it to use if a schema is set, stored as $this->metaKeysSQL1 and $this->metaDefaultsSQL1,
 * respectively.
 *
 * Overwrite written by Ron Stewart on July 14, 2010
 * Moodle file to replace: /lib/adodb/drivers/adodb-postgres64.inc.php
 * Reason: The functions &MetaColumns and &MetaIndexes were gathering database wide metatdata for whichever
 * Moodle table was being passed to them. This resulted in n records per column in the table, where n = the
 * number of eschool schemas in the database.
 * Modifications: In both cases, I added $CFG as a global variable, and then set $schema = $CFG->eschool->shortname
 * to take advantage of the already existing schema support in this file.
 *
...
WHERE relkind in ('r','v') AND (c.relname='%s' or c.relname = lower('%s'))
 and c.relnamespace=n.oid and n.nspname='%s' 
	...
	// OVERWRITE 1: inserted to file, starting here.
	...
	FROM pg_class bc, pg_class ic, pg_index i, pg_attribute a, pg_namespace n WHERE ic.relnamespace=n.oid and n.nspname='%s' and bc.oid = i.indrelid AND ic.oid = i.indexrelid AND (i.indkey[0] = a.attnum OR i.indkey[1] = a.attnum OR i.indkey[2] = a.attnum OR i.indkey[3] = a.attnum OR i.indkey[4] = a.attnum OR i.indkey[5] = a.attnum OR i.indkey[6] = a.attnum OR i.indkey[7] = a.attnum) AND a.attrelid = bc.oid AND bc.relname = '%s'";
	// END OVERWRITE 1
	...
	// OVERWRITE 2: inserted to file, starting here.
	var $metaDefaultsSQL1 = "SELECT d.adnum as num, d.adsrc as def from pg_attrdef d, pg_class c, pg_namespace n where c.relnamespace=n.oid and n.nspname='%s' and d.adrelid=c.oid and c.relname='%s' order by d.adnum";
	// END OVERWRITE 2
...
select viewname,'V' from pg_views where viewname like $mask";
	...
	*/
	...
	{
		// OVERWRITE 3: updated file, changed from:
		// global $ADODB_FETCH_MODE;
		global $ADODB_FETCH_MODE, $CFG;
	
		$table = $CFG->current_app->getShortName() . '.' . $table;
		if (isset($CFG->metacolumns_cache[$table]))
		{
			return $CFG->metacolumns_cache[$table];
		}
		// END OVERWRITE 3
		$schema = false;
		$false = false;
		...
		if (!empty($this->metaKeySQL)) {
			...
			// OVERWRITE 4: updated from:
			// $rskey = $this->Execute(sprintf($this->metaKeySQL, ($table)));
			if ($schema)
			{
				$rskey = $this->Execute(sprintf($this->metaKeySQL1, $schema, $table));
			}
			else
			{
				$rskey = $this->Execute(sprintf($this->metaKeySQL, $table));
		...
		if (!empty($this->metaDefaultsSQL)) {
			$ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
			// OVERWRITE 5: updated from:
			// $sql = sprintf($this->metaDefaultsSQL, ($table));
			if ($schema)
			{
				$sql = sprintf($this->metaDefaultsSQL1, $schema, $table);
			}
			else
			{
			$sql = sprintf($this->metaDefaultsSQL, ($table));
		...
		{ // OVERWRITE 6: updated from: return $retarr
			$CFG->metacolumns_cache[$schema . '.' . $table] = $retarr;
			return $retarr;	
                }       
