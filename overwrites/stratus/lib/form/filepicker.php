...
class MoodleQuickForm_filepicker extends HTML_QuickForm_input {
    ...
    function exportValue(&$submitValues, $assoc = false) {
        ...
        if (!is_null($draftitemid)) {
            // GC OVERWRITE - group creation - if no picture is included, the above conditional sets the $draftitemid
            // as an empty string.  The empty string is not null, so this is called.  When the $fs->get_area_files
            // function is called, it fails because $draftitemid is empty, not false, and the database throws an error.
            // I'm including a further check to make sure that the value is filled.
