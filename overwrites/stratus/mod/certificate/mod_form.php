...
class mod_certificate_mod_form extends moodleform_mod {
    ...
    function definition() {
        ...
        if ($CFG->current_app->hasPrivilege('EschoolStaff'))
        {
            ...
            if (empty($modid))
            {
                $param = 'courseid=' . optional_param('course', 0, PARAM_INT);
            }
            else
            {
                $param = 'modid=' . $modid;
            }
            $mform->addElement('static', 'addimages', '<a href="' . $CFG->wwwroot . '/mod/certificate/upload_image.php?' . 
                    $param . '"><b>Upload Images</b></a>');
        }
        // END OVERWRITE 1
