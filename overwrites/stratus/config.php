...
$CFG->performance_ts = time();
$CFG->metacolumns_cache = array();
$CFG->disableupdatenotifications = true;
...
// gcr::initializeEschool($url[0]) loads the symfony library files, allowing access to Doctrine 
// and it builds the global eschool object $CFG->eschool

// Here we cheack to see if this file is being called as part of a client script. In order
// to navigate n schemas, we need to pass the --eschool argument to any moodle script which
// is ran from the command line.
$shortname = false;
if (isset($argv))
{
...
else if (gcr::initialize())
{
    if ($CFG->current_app->isMahara())
    {
        // the url is bad because it points to the stratus folder on a mahara short_name
        header('Location: ' . $CFG->current_app->getAppUrl());
        die();
    }
    $CFG->dbtype    = 'pgsql';      // 'pgsql', 'mysqli', 'mssql' or 'oci'
    $CFG->dblibrary = 'native';     // 'native' only at the moment
    $CFG->dbhost    = gcr::DBHostName;  // eg 'localhost' or 'db.isp.com' or IP
    $CFG->dbname    = gcr::DBName;     // database name, eg moodle
    $CFG->dbuser    = 'gc4' . $CFG->current_app->getShortName() . 'admin';   // your database username
    $CFG->dbpass    = $CFG->current_app->getAdminPassword();   // your database password
    $CFG->prefix    = gcr::moodlePrefix;  // prefix to use for all table names
    $CFG->dboptions = array(
        'dbpersist' => false,       // should persistent database connections be
                                    //  used? set to 'false' for the most stable
                                    //  setting, 'true' can improve performance
        ...
        'dbsocket'  => false,       // should connection via UNIX socket be used?
                                    ...
                                    //  if dbhost is 'localhost' - if you need
                                    //  local port connection use '127.0.0.1')
        'dbport'    => gcr::DBPort, // the TCP port number to use when connecting
                                    //  to the server. keep empty string for the
                                    //  default port
        'dbschema' => $CFG->current_app->getShortName(),
    );

    $CFG->wwwroot = $CFG->current_app->getAppUrl();
    $CFG->symfonyroot = $CFG->current_app->getUrl();
    $CFG->dirroot   = substr(gcr::moodleDir, 0, strlen(gcr::moodleDir) - 1);
    $CFG->dataroot  = gcr::moodledataDir . $CFG->current_app->getShortName();
    $CFG->admin     = 'admin';
    $CFG->directorypermissions = 00777;  // try 02777 on a server in Safe Mode

    // set the passwordsaltmain to this eschool's stored salt
    $CFG->passwordsaltmain = $CFG->current_app->getPasswordSalt();
    $CFG->current_app->loadPasswordSalts();

    // optional config settings in use.

    $CFG->tracksessionip = false;
    $CFG->mnetkeylifetime = 4000;
    // $CFG->disablestatsprocessing = true; make this dynamic
    $CFG->dblogerror = true;
    $CFG->emailconnectionerrorsto = 'sysadmin@globalclassroom.us';
    // $CFG->defaulthtmleditor='tinymce'; No longer a setting in 2.0
    $CFG->allowthemechangeonurl = false;
    
    //$CFG->extramemorylimit = '1G'; // new setting on 2.0 that we probably want turned on

    // Moodle 1.9 optional config settings not currently being used
    ...
    // Possible settings on Moodle 2.0
...
//      $CFG->disablestatsprocessing = true;
//      $CFG->admineditalways = true;
    $CFG->defaultblocks_override = 'course_profile,activity_modules,invite_to_course';
//      $CFG->defaultblocks_site = 'site_main_menu,admin,course_list:course_summary,calendar_month';
//      $CFG->defaultblocks_social = 'participants,search_forums,calendar_month,calendar_upcoming,social_activities,recent_activity,admin,course_list';
//      $CFG->defaultblocks_topics = 'participants,activity_modules,search_forums,admin,course_list:news_items,calendar_upcoming,recent_activity';
//      $CFG->defaultblocks_weeks = 'participants,activity_modules,search_forums,admin,course_list:news_items,calendar_upcoming,recent_activity';
...
// $CFG->showcrondebugging = true;
    ...
    // In Moodle 2.0 global variables are handled differently upon setup, so in order
    // to maintain them for use in symfony pages, we need to reset them here:
    $CFG->current_app->initializeCurrentApp($USER, $SESSION, $SITE);
}
