...
require_once("$CFG->dirroot/enrol/globalclassroom/editenrolment_form.php");
...
// Make sure the user can manage globalclassroom enrolments for this course
require_capability("enrol/globalclassroom:manage", get_context_instance(CONTEXT_COURSE, $course->id, MUST_EXIST));
...
$url = new moodle_url('/enrol/globalclassroom/editenrolment.php', $returnurl->params());
...
if (!$plugin->allow_manage($instance) || $instance->enrol != 'globalclassroom' || !($plugin instanceof enrol_globalclassroom_plugin)) {
    print_error('erroreditenrolment', 'enrol');
}

$mform = new enrol_globalclassroom_user_enrolment_form($url, array('user'=>$user, 'course'=>$course, 'ue'=>$ue));
...
$title = get_string('editenrolment', 'enrol_globalclassroom');
